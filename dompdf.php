...
$sql2 = "SELECT  A1.CITY,A1.NAIMUL,A.DOM,A.KVARTIRA FROM t1.spr_abon a, t1.SPR_Ulic a1  WHERE a.KOD ='$login'
...
if ($table3[0]['SALDO_PRED'] < 0) {
    $debt = 0;
    $overpayment = $table3[0]['SALDO_PRED'];
} elseif ($table3[0]['SALDO_PRED'] > 0) {
    $overpayment = 0;
    $debt = $table3[0]['SALDO_PRED'];
} elseif ($table3[0]['SALDO_PRED'] == 0) {
    $debt = 0;
    $overpayment = 0;
...
$html = include_template('template.php', [
    ...
    'DEPT' => $debt,
    'OVERPAY' => $overpayment,
    'DATE' => $table3[0]['DATE_LAST_OPLAT'],
    'SUMMA_OPLAT' => $table3[0]['SUMMA_OPLAT'],
    'SUMMA_ALL' => $table3[0]['SUMMA_ALL'],
    'GOSP' => $table3[0]['GOSP'],
    'FIO' => toUtf($table3[0]['FIO']),
    'S' => $table3[0]['S'],
    'ADDRESS' => $adrres,
    'CountTenants' => $table3[0]['KOLJIL'],
    'vodomers' => $vodomers,
    'table3' => $table3

]);
$email = 'ivan701-1992@mail.ru';
use Dompdf\Dompdf;
...
if (!empty($email)) {
    ...
    $file = $output = $dompdf->output();
    $mailTo = $email;
    $from = "gorvodokanal@.com";
    $subject = "=?windows-1251?B?". base64_encode("Подтверждение электронной почты"). "?=";
    $message = "Файл ЕПД для абанента-". $login;
...
// второй частью прикрепляем файл, можно прикрепить два и более файла
    ...
    mail($mailTo,$subject,$multipart,$mailheaders);
// отправляем письмо
