...
$sql3 = "SELECT       
      ...
      (SELECT koeff FROM t1.koeff WHERE to_date('$today','dd.mm.yyyy') BETWEEN date_begin AND date_end) povish_koef,
       NVL(DECODE(vid_uslugi,1,(SELECT sum(nvl(n.sumo,0))  FROM t1.nachis n WHERE date_ot=to_date('$today','dd.mm.yyyy') AND kod=kod1  AND vid_uslugi=19),3,0,(SELECT sum(nvl(n.sumo,0))  FROM t1.nachis n WHERE date_ot=to_date('$today','dd.mm.yyyy') AND kod=kod1  AND vid_uslugi=19)),0) sum_povish,
...
$str1 = $row['FIO'];
$str = mb_convert_encoding($str1, "UTF-8", "ASCII");


$str3 = $row['USL'];
$str2 = mb_detect_encoding($str3);
