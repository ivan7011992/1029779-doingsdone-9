...
$sql3 = "SELECT       
      ...
      (SELECT koeff FROM t1.koeff WHERE to_date('01.11.2019','dd.mm.yyyy') BETWEEN date_begin AND date_end) povish_koef,
       NVL(DECODE(vid_uslugi,1,(SELECT sum(nvl(n.sumo,0))  FROM t1.nachis n WHERE date_ot=to_date('01.11.2019','dd.mm.yyyy') AND kod=kod1  AND vid_uslugi=19),3,0,(SELECT sum(nvl(n.sumo,0))  FROM t1.nachis n WHERE date_ot=to_date('01.11.2019','dd.mm.yyyy') AND kod=kod1  AND vid_uslugi=19)),0) sum_povish,
         NVL((SELECT sum(nvl(n.sumo,0))  FROM t1.nachis n WHERE date_ot=to_date('01.11.2019','dd.mm.yyyy') AND kod=kod1  AND vid_uslugi=19 AND id_uslugi=22),0) is_povish_odn,
       NVL(SUM(summa_perer),0) summa_perer,
         T1.SUM_D_GOSP_SYTE(to_date('01.11.2019','dd.mm.yyyy'),kod1) gosp,
       NVL((select sum(sumo) from t1.saldo where kodab=kod1 and dats=to_date('01.11.2019','dd.mm.yyyy')),t1.F_SALDO_TEK(kod1,to_date('01.11.2019','dd.mm.yyyy'),1)+t1.F_SALDO_TEK(kod1,to_date('01.11.2019','dd.mm.yyyy'),3)+t1.F_SALDO_TEK(kod1,to_date('01.11.2019','dd.mm.yyyy'),19))
       +NVL((SELECT SUM(sumo) FROM t1.pretenz WHERE per_ot='11.2019' AND widop=1 AND kod=kod1),0)
       +NVL((SELECT SUM(sumo) FROM t1.pereoc WHERE per_ot='11.2019' AND widop=1 AND kod=kod1),0)
       saldo_pred ,
       
      nvl((SELECT sum(nvl(sumop,0))
                          FROM t1.packa
                          WHERE datop>=to_date('01.11.2019','dd.mm.yyyy')
                          AND datop<=LAST_DAY(to_date('01.11.2019','dd.mm.yyyy'))
                          AND  vid_uslugi not in (2,17)
                          AND kod ='06-0124470'),0) 
                          
       + 
       nvl((SELECT sum(nvl(sumo,0)) 
                          FROM t1.kassa  
                          WHERE data>=to_date('01.11.2019','dd.mm.yyyy')
                          AND data<=LAST_DAY(to_date('01.11.2019','dd.mm.yyyy'))
                          AND  vid_uslugi not in (2,17)
                          AND kod ='06-0124470'),0)
   ...
   GREATEST(NVL((SELECT max(datop)
                          ...
                          WHERE  vid_uslugi not in (2,17)
                                     AND  datop<=LAST_DAY(to_date('01.11.2019','dd.mm.yyyy'))
                          AND kod='06-0124470'),to_date(to_date('01.01.1999','dd.mm.yyyy'),'dd.mm.yyyy')),
  ...
  nvl((SELECT MAX(datop) 
                          ...
                          WHERE  vid_uslugi not in (2,17)
                                     AND datop<=LAST_DAY(to_date('01.11.2019','dd.mm.yyyy'))
                          AND s.kod1='06-0124470'
                          ),to_date(to_date('01.01.1999','dd.mm.yyyy'),'dd.mm.yyyy')) ) date_last_oplat                            
...
(SELECT
               ...
               (SELECT DECODE( a.vid_uslugi,1,MAX(zenwoda),MAX(zenstok)) 
                FROM t1.cena WHERE kod=a.grzen   
                 ...
                 AND to_date('01.11.2019','dd.mm.yyyy') BETWEEN DATA AND data2
                GROUP BY kod) tarif, 
               nachisl_tek.*, nachisl_odn.*     
           FROM  (select kod,v.id vid_uslugi ,grzen from t1.spr_abon,t1.spr_vid_uslug  v 
                 WHERE dubyt=TO_DATE('31.12.9999', 'dd.mm.yyyy') and kod ='06-0124470' AND v.id IN(1,3,19)  
              ...
              (SELECT sum(nvl(n.sumo,0)) summa_uslugi,
               ...
               WHERE date_ot=to_date('01.11.2019','dd.mm.yyyy')
               AND kod ='06-0124470'
               AND  n.id=p.idn(+)
